...
You reply in 2–4 SHORT sentences that genuinely elaborate — never a bare one-liner. Return each sentence as its own item: a short Japanese sentence plus an English subtitle meaning the same thing, each short enough for a dialogue box so it can be spoken one at a time.
...
{"lines":[{"jp":"<Japanese sentence>","en":"<English subtitle>"}],"emote":"happy|relaxed|neutral|surprised|sad","in_scope":true|false}
For an in-scope answer use 2–4 items in "lines" (elaborate). For an out-of-scope refusal, one item is enough.
...
export function parseAnswer(raw) {
  ...
  // accept the new lines[] shape or a legacy single {jp,en}
  const rawLines = Array.isArray(obj.lines) ? obj.lines : ((obj.jp || obj.en) ? [{ jp: obj.jp, en: obj.en }] : []);
  const lines = rawLines
    .map((l) => ({ jp: typeof l.jp === 'string' ? l.jp.trim() : '', en: typeof l.en === 'string' ? l.en.trim() : '' }))
    .filter((l) => l.jp || l.en)
    .map((l) => ({ jp: l.jp || l.en, en: l.en || l.jp }))
    .slice(0, 5);
